...
func resizeList(myList []int, newSize int) []int {
	// Add 0 to expand memory
	max := newSize - len(myList) + 2
...
func extractParameters(myList []int, index int, relativeBase int) ([]int, int, int, int, int) {
	...
	if opcode == 99 {
		...
		if index+1 < len(myList) {
			...
			case 2:
				if myList[index+1] < len(myList) {
					// Test if memory is outside existing
					if relativeBase+myList[index+1] > len(myList) {
						myList = resizeList(myList, relativeBase+myList[index+1])
					}
					param1 = myList[relativeBase+myList[index+1]]
					if opcode == 3 {
						param1 = relativeBase + myList[index+1]
					}
				} else {
					param1 = 0
				}
			...
			// Check if continue retrieving parameters
				...
				if index+2 < len(myList) {
					...
					case 0:
						if myList[index+2] > len(myList) {
							myList = resizeList(myList, myList[index+2])
						}
					...
					case 2:
						if myList[index+2] < len(myList) {
							// Test if memory is outside existing
							if relativeBase+myList[index+2] > len(myList) {
								myList = resizeList(myList, relativeBase+myList[index+2])
							}
							param2 = myList[relativeBase+myList[index+2]]
						} else {
							param2 = 0
						}
						break
					...
					if (opcode == 5) || (opcode == 6) {
						...
						if index+3 < len(myList) {
							...
							case 0:
								...
								break
							case 2:
								param3 = relativeBase + myList[index+3]
								break
							}
							// Test if memory is outside existing
							if param3 > len(myList) {
								myList = resizeList(myList, param3)
							}
						}
					}
				}
	...
	return myList, opcode, param1, param2, param3
...
func runProgram(myList []int, val1 int) {
	...
	for continuer {
		...
		myList, opcode, param1, param2, param3 = extractParameters(myList, index, relativeBase)
